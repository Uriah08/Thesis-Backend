...
class FarmDashboardSerializer(serializers.ModelSerializer):
    session_counts_by_day = serializers.SerializerMethodField()
    ...
    def get_session_counts_by_day(self, obj):
        sessions = (
            FarmSessionModel.objects
            .filter(farm=obj)
        ...
        return [{'created_at': s['day'], 'count': s['count']} for s in sessions]
    ...
    def get_session_trays_count_by_day(self, obj):
        session_trays = (
            ...
            .filter(farm=obj, finished_at__isnull=False)
    ...
    def get_detected_and_reject_by_day(self, obj):
        daily_stats = (
            SessionTrayModel.objects
            .filter(farm=obj, created_at__isnull=False)
            .annotate(day=TruncDate('created_at'))
            .values('day')
            .annotate(
        ...
        return [
            {
                ...
                "session_name": tray.session.name if tray.session else None,
                "tray_id": tray.tray.id if tray.tray else None,
                "session_id": tray.session.id if tray.session else None,
            }
            for tray in trays
        ]
